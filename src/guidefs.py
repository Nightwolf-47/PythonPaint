    ...
    if event.type == pygame_gui.UI_FILE_DIALOG_PATH_PICKED:
        if event.ui_object_id == "savefile":
            if paint.save_image(event.text):
                pygame_gui.windows.UIMessageWindow(pygame.Rect(0,0,640,480),"File saved successfully!",manager,window_title="Save File Success")
            else:
                pygame_gui.windows.UIMessageWindow(pygame.Rect(0,0,640,480),"File couldn't be saved!",manager,window_title="Save File Failure")
